...
void mqttCallback(char* topic, uint8_t* payload, unsigned int length) {
  ...
  else if(targetStr == "broadcast") 
  {
    ...
    // TODO : The boradcast of the received mesh ID and password over the network and 
    //        The algorithm to ensure that all the nodes have recieved the password 
    //        then send restart command to all leaf nodes and then write into EEPROM of root and then restart
    // If due to power cycle during this process we need to have the root node still working on the old mesh ID and PW 
    // so that any node that did not receive the ID and password can still be programmed with the new mesh ID and pW
    // And once all nodes have received new ID and PW we can then got to restart at the root node
  }
  else
  {
